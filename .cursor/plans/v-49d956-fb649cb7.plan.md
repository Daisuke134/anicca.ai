<!-- fb649cb7-68ad-4f47-87e4-f254aacf033f 9b5fe41d-933d-4884-8dd7-a415c7c13351 -->
...
- 内容:
  ...
  - ただし現状は **OpenAIへのSDP交換がHTTP 400で落ちており接続未成立**。根本原因として、バックエンドが発行したclient_secretの`session.model`（例: `gpt-4o-realtime-preview` / `gpt-realtime-mini`）と、iOS側が固定で使っているクエリ `model=gpt-realtime` が不一致になり得る（iOSはレスポンスの`session`をデコードしておらず `payload.model` も無視している）。
    - 対応: バックエンドはレスポンスに `model` を明示的に含め、iOSは `sessionModel = payload.model ?? "gpt-realtime"` として **同じモデル名で `/v1/realtime?model=...` に投げる**。
  - 併せて、`/v1/realtime` の非2xx時にレスポンス本文をログに出して原因を即特定できるようにする（今はステータスしか出ていない）。
...
## 1) Realtime: `Realtime SDP exchange failed with status 400` / `Session ended`（根治パッチ）

### 原因（コード根拠）
- **バックエンド**は `OPENAI_REALTIME_MODEL`（デフォルト `gpt-realtime`）で client_secret を発行している  
  → `apps/api/src/services/openaiRealtimeService.js` の `REALTIME_MODEL`（L8）  
- **iOS側**は Realtime SDP交換に使う `model` を **固定で `gpt-realtime` に上書き**している  
  → `aniccaios/aniccaios/VoiceSessionController.swift` の `sessionModel = "gpt-realtime"`（L359 近辺）  
- もし Railway(Staging)で `OPENAI_REALTIME_MODEL=gpt-4o-realtime-preview` 等になっていると、**client_secret発行モデルとiOSがSDP交換で指定するmodelが不一致**になり、`/v1/realtime?model=...` が **400** になり得ます。

### 直し方（方針）
- **サーバ**が「実際に発行したモデル名」を `model` として返す  
- **iOS**がそれを使って `/v1/realtime?model=<same>` でSDP交換する  
- さらに、非2xxのとき **レスポンス本文をログ**に出して即原因特定できるようにする

### 擬似パッチ

```diff
diff --git a/apps/api/src/services/openaiRealtimeService.js b/apps/api/src/services/openaiRealtimeService.js
index 1111111..2222222 100644
--- a/apps/api/src/services/openaiRealtimeService.js
+++ b/apps/api/src/services/openaiRealtimeService.js
@@ -1,54 +1,66 @@
...
+    // IMPORTANT: iOS must use the same model for /v1/realtime SDP exchange, otherwise it can 400.
...
```diff
diff --git a/apps/api/src/routes/mobile/realtime.js b/apps/api/src/routes/mobile/realtime.js
index 3333333..4444444 100644
--- a/apps/api/src/routes/mobile/realtime.js
+++ b/apps/api/src/routes/mobile/realtime.js
@@ -50,6 +50,7 @@ router.get('/session', async (req, res) => {
     return res.json({
       ...payload,
+      model: payload.model,
...
```diff
diff --git a/aniccaios/aniccaios/VoiceSessionController.swift b/aniccaios/aniccaios/VoiceSessionController.swift
...
+        // IMPORTANT:
+        // Use model returned by backend (which mirrors the model used to create the ephemeral client secret).
+        // If iOS uses a different model in /v1/realtime?model=..., SDP exchange can fail with HTTP 400.
...
+                // Log body for diagnosis (often includes invalid_value / invalid_model / etc.)
+                let body = String(data: data, encoding: .utf8) ?? "<non-utf8>"
+                logger.error("Realtime SDP exchange failed: status=\(http.statusCode) body=\(body, privacy: .public)")
             }
             cachedSecret = nil
...
## 2) BGTask: `Registration rejected; com.anicca.metrics.daily is not advertised in the application's Info.plist`

### 原因（コード根拠）
- `AppDelegate` が起動時に `BGTaskScheduler.register(forTaskWithIdentifier: MetricsUploader.taskId, ...)` を呼んでいる  
  → `aniccaios/aniccaios/AppDelegate.swift`（L24–37）
- `MetricsUploader.taskId` は `com.anicca.metrics.daily`  
  → `aniccaios/aniccaios/Services/MetricsUploader.swift`（L19）
- しかし `Info.plist` に `BGTaskSchedulerPermittedIdentifiers` が無い  
  → `aniccaios/aniccaios/Info.plist`（現状）

### 擬似パッチ

```diff
diff --git a/aniccaios/aniccaios/Info.plist b/aniccaios/aniccaios/Info.plist
...
@@ -1,68 +1,76 @@
 ...
 <dict>
   ...
   <string>$(INFOPLIST_KEY_ANICCA_PROXY_BASE_URL)</string>
@@
   <key>UIApplicationSceneManifest</key>
   <dict/>
+
+  <!-- BGTaskScheduler: must include permitted identifiers to avoid "Registration rejected" -->
...
- [ ] Realtime SDP交換400の根治: backendがmodelを返しiOSがそのmodelで/realtimeに接続。非2xx時は本文もログ出し。
...
- [ ] BGTaskの登録拒否(com.anicca.metrics.daily)を解消するため、Info.plistにBGTaskSchedulerPermittedIdentifiersを追加する