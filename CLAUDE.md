...
### 1.5. リリース管理ルール（Trunk-Based Development）

**ブランチ構成:**
```
main          = App Store で承認・公開済みのコード
release/x.x.x = App Store 提出済み・レビュー中のコード
dev           = 開発中のコード（ここで全ての開発を行う）
```

**フロー:**
1. `dev` で開発（feature ブランチは作らない）
2. 完成したら `release/x.x.x` ブランチを切る
3. TestFlight でテスト → App Store 提出
4. **承認されるまで main にマージしない**
5. 承認後 `release/x.x.x` → `main` にマージ

**バージョニング（Semantic Versioning）:**
- `MAJOR.MINOR.PATCH` 形式（例: 1.1.0）
- PATCH（1.0.x）: バグ修正
- MINOR（1.x.0）: 新機能追加（後方互換あり）
- MAJOR（x.0.0）: 破壊的変更（後方互換なし）

**後方互換ルール:**
- 古いコードを消す前に **最低2〜3バージョン待つ**
- 新旧両形式をサポートする移行期間を設ける
- 理由: 古いバージョンのユーザーのアプリが壊れるのを防ぐ

**現在のバージョン状況:**
| バージョン | 状態 | ブランチ |
|-----------|------|---------|
| 1.0.8 | レビュー中 | main |
| 1.1.0 | 開発完了（Phase 4） | dev |
